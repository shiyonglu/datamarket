...
window.addEventListener('load', async () => {
    ...
    currentPrice.innerHTML = "DAT " + price
    if (status == "1") {
...
purchaseDataset.addEventListener('click', async () => {
    if (sessionStorage.getItem("connected") !== "true"){
        ...
        else if (parseFloat(web3.utils.fromWei(datBalance, 'ether')) < parseFloat(price)) {
            window.alert("DAT balance is not enough!"); // compare DAT balance
        }
        else {
            try{
                var spender = await DATContract.methods.approve(contractAddress, web3.utils.toWei(price, 'ether')).send({
